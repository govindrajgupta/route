...
with st.sidebar:
    ...
    current_battery_range = st.slider("Current Battery Range (km)", 5.0, 50.0, BATTERY_RANGE, step=1.0)
...
if find_button:
    if start_loc == end_loc:
        ...
        if cost == float("inf"):
            ...
            with col2:
                ...
                # Display route coordinates as a table instead of map (to avoid pandas dependency)
                st.markdown("**Stop Coordinates:**")
                for i, city in enumerate(path, 1):
                    if city in COORDS:
                        lat, lon = COORDS[city]
                        st.markdown(f"{i}. **{city}**: `{lat:.4f}°N, {lon:.4f}°E`")
                st.caption(f"Showing coordinates for {len(path)} stops.")
    ...
    with col2:
        ...
        # Display all location coordinates as text (to avoid pandas dependency)
        for name, (lat, lon) in sorted(COORDS.items()):
            st.markdown(f"• **{name}**: `{lat:.4f}°N, {lon:.4f}°E`")